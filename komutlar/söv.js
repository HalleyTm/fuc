exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "466872033230454799")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Yapımcıma Sövemezsin!!! Belanı...`)
}}) 
   if(member.id === "528885635985965058")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Arkadaş Bota Sövemessin!!!`)
}}) 
  if(member.id === "466872033230454799")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop Bana Sövemezsin!!!`)
}})
  if(member.id === "442771345210867712")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop Bedo Başkana Sövemessin!!!`)
}}) 
    if(member.id === "442618153848012801")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop Amdin Bey Kerim'e Sövemessin At_Kafası!!!`)
}})
